...
class FnRMatch extends Component {
  ...
  render() {
    ...
    const espnLogo = <img src="http://a.espncdn.com/espn3/images/watchespn/slideshow_logo_espn3.png" title="Watch Espn" />

    let stream = match.espnstreamId  // If match has a espnstreamId
      ? <div className="FnR-match-espn">
        <Link to={`http://www.espn.com/watchespn/index/_/sport/rugby#type/upcoming/`} target="_blank">
          {espnLogo}
        </Link>
      </div>
      : <div className="FnR-match-espn"></div>
    ...
    if (match.status.includes('FINAL') || match.status.includes('UNDERWAY') || match.status.includes('HALFTIME')) {
      ...
      stream = match.espnstreamId ?   //Change ESPN logo to send to actual video stream.
        <div className="FnR-match-espn">
          <Link to={`http://www.espn.com/watchespn/player/_/id/${match.espnstreamId}/`} target="_blank">
            {espnLogo}
          </Link>
        </div>
        : null
      ...
      const post = results
        ...
        : this.generatePost(match, teams,
            <div className="FnR-match-time">
              <FormattedDate value={new Date(match.date)} year='numeric' month='short' day='2-digit'/>
            </div>)
      return (
          <div className={activeMatch} key={match.id}>
            <div className="FnR-match-teamData">
              <div className="FnR-match-team">
                <img className="FnR-match-teamLogo" src={homeLogo} title={homeTeam.displayName}/>
                {match.homeTeam.abbr}
              </div>
              ...
              <div className="FnR-match-team">
                <img className="FnR-match-teamLogo" src={visitingLogo} title={visitingTeam.displayName}/>
